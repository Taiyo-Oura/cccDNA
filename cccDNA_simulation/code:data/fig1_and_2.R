...

{
  ...
  add_day0_dna <- function(df_list, y0_estimated) {
    lapply(df_list, function(df) {

      new_row <- df[1, ]
      new_row[ , names(new_row) %in% c("x")] <- 0

      if ("y" %in% names(new_row))    new_row$y    <- y0_estimated
      if ("mean" %in% names(new_row)) new_row$mean <- y0_estimated
      if ("ymin" %in% names(new_row)) new_row$ymin <- y0_estimated
      if ("ymax" %in% names(new_row)) new_row$ymax <- y0_estimated
  ...
  d.list <- add_day0_dna(d.list, y0_estimated = log10(mean(parset$y0)))
